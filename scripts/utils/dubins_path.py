...
#from reeds_shepp import * # fix please!
...
class ReedsSheppPath:
    ...
    def path_reformat(start, turning_radius, car, path_type_data):
        path_segments = []
        ...
        for seg_type, seg_length in path_type_data:
#            if abs(seg_length * turning_radius) < 1e-3:
#                continue
            if seg_type == 0:
                continue
            elif seg_type == 1:
                phi = car.max_phi # left
            elif seg_type == 2:
                phi = 0
            elif seg_type == 3:
                phi = -car.max_phi # right
            else:
                raise ValueError(f"Unknown segment type: {seg_type}")

            mode = 1 if seg_length > 0 else -1

            # Compute new pose from current one
            # Basic integration over the segment:
            L = abs(seg_length)   # true arc length
            if phi == 0:
                dx = L * np.cos(pose[2]) * mode * turning_radius
                dy = L * np.sin(pose[2]) * mode * turning_radius
                dtheta = 0
            else:
                R = car.l / np.tan(abs(phi))
                dtheta = L / R * np.sign(phi) * mode
                dx = R * (np.sin(pose[2] + dtheta) - np.sin(pose[2])) * np.sign(phi) * mode
                dy = -R * (np.cos(pose[2] + dtheta) - np.cos(pose[2])) * np.sign(phi) * mode
            
            new_pose = pose + np.array([dx, dy, dtheta])
            new_pose[2] = new_pose[2] % (2 * np.pi)

            path_segments.append((new_pose.tolist(), phi, mode))
            pose = new_pose  # update for next segment
            #print(f"Segment:\n  Direction:{seg_type}\n  Gear:{mode}\n   Distance:{L}\n  End:{new_pose}")
        return path_segments

    def find_path(self, start_pos, end_pos):
        """ Get the shortest obstacle-free dubins path. """
        points = path_sample(start_pos, end_pos, self.r, 0.02)
        for point in points:
            vertex = self.car.get_car_bounding(point[:3])
            if not self.car.env.rectangle_safe(vertex):
                return None, None, False
        #print("Path type: ", path_type(start_pos, end_pos, self.r))
        #print("Path length: ", path_length(start_pos, end_pos, self.r))
        #print("Reformatted type: ", ReedsSheppPath.path_reformat(start_pos, self.r, self.car, path_type(start_pos, end_pos, self.r)))
        #print("Some samples: ", path_sample(start_pos, end_pos, self.r, 0.05))
        #return ReedsSheppPath.path_reformat(start_pos, self.r, self.car, path_type(start_pos, end_pos, self.r)), path_length(start_pos, end_pos, self.r), True
        return [(x[0:3], x[3], 1 if x[4] > 0 else -1) for x in points], path_length(start_pos, end_pos, self.r), True
